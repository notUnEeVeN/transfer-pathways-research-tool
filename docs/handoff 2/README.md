...
2. **`PREREQ-GRAPH-HANDOFF.md`** — addendum for the prerequisite graph (Jul 16): swim-lane redesign. **`PrereqGraph.jsx`** is the shippable component (React, zero deps); `mockups/Prereq Graph.dc.html` is its interactive demo.
...
4. **`mockups/`** — the two approved interactive prototypes. **These are the visual source of truth**; `…v2` wins any conflict with prose.
   ...
   - `Prereq Graph.dc.html` — swim-lane prerequisite graph demo (canonical + sample-college modes, click-to-trace focus).
