/* ============================================================================
   ...
   Exports: PrereqGraph, PrereqGraphDemo, computePrereqLayout, CANONICAL_CONCEPTS.
   ========================================================================== */
...
function computePrereqLayout(nodes, reqEdges, { compact }) {
...
function PrereqGraph({ mode = 'canonical', concepts, rules, courses, edges, conceptIndex }) {
  const data = useMemo(() => {
    if (mode === 'college') {
      const ci = conceptIndex || CANONICAL_INDEX;
    ...
    const cs = concepts || CANONICAL_CONCEPTS;
...
/* ---- canonical mock data -------------------------------------------------- */
const RAW = `calc_1|math|Calculus I|
calc_2|math|Calculus II|calc_1
calc_3|math|Calculus III (multivariable)|calc_2
linear_alg|math|Linear Algebra|calc_3
diff_eq|math|Ordinary Differential Equations|linear_alg
discrete_math|math|Discrete Mathematics|calc_1
bus_calc_1|math|Applied/Business Calculus I|
bus_calc_2|math|Applied/Business Calculus II|bus_calc_1
stats_calc|stats|Calc-Based Probability & Statistics|calc_2
intro_stats|stats|Introductory Statistics|
phys_mech|physics|Physics: Mechanics (calc-based)|calc_1
phys_em|physics|Physics: E&M (calc-based)|phys_mech,calc_2
phys_waves_thermo|physics|Physics: Waves/Thermo/Optics|phys_em
phys_modern|physics|Physics: Modern|phys_waves_thermo
phys_gen_1|physics|General Physics I (algebra-based)|
phys_gen_2|physics|General Physics II (algebra-based)|phys_gen_1
gen_chem_1|chem|General Chemistry I|
gen_chem_2|chem|General Chemistry II|gen_chem_1
organic_chem_1|chem|Organic Chemistry I|gen_chem_2
organic_chem_2|chem|Organic Chemistry II|organic_chem_1
cs_1|cs|Programming Fundamentals (CS1)|
cs_2_oop|cs|OOP / CS2|cs_1
cs_3_data_structures|cs|Data Structures & Algorithms (CS3)|cs_2_oop
comp_arch_assembly|cs|Computer Org & Assembly|cs_1
c_systems_programming|cs|C / Systems Programming|cs_1
digital_logic|engr|Digital Logic Design|cs_1
bio_cell_molec|bio|Cell & Molecular Biology|
bio_organismal|bio|Organismal Biology|bio_cell_molec
human_physiology|bio|Human Physiology|bio_cell_molec,gen_chem_1
engr_circuits|engr|Intro Circuit Analysis|phys_em,diff_eq
engr_programming|engr|Programming for Engineers|calc_1
engl_comp_1|other|English Composition 1|
engl_comp_2|other|English Composition 2|engl_comp_1
public_speaking|other|Public Speaking|
econ_micro|other|Microeconomics|
econ_macro|other|Macroeconomics|
acct_financial|other|Financial Accounting|
intro_lit|other|Intro to Literature|engl_comp_1
world_lit_1|other|World Literature I|engl_comp_1
world_lit_2|other|World Literature II|engl_comp_1
linear_alg_diff_eq|math|Linear Alg + Diff Eq (combined)|calc_3|linear_alg,diff_eq`;

const CANONICAL_CONCEPTS = RAW.split('\n').map(line => {
  const [slug, discipline, name, req, sat] = line.split('|');
  return { slug, discipline, name, requires: req ? req.split(',') : [], satisfies: sat ? sat.split(',') : [] };
});
const CANONICAL_INDEX = Object.fromEntries(CANONICAL_CONCEPTS.map(c => [c.slug, c]));

/* ---- sample college (mode 2 demo): edges projected from concept rules ----- */
const SAMPLE_COLLEGE_COURSES = [
  ['MATH-3A', 'MATH', '3A', 'Calculus and Analytic Geometry I', 'calc_1'],
  ['MATH-3B', 'MATH', '3B', 'Calculus and Analytic Geometry II', 'calc_2'],
  ['MATH-3C', 'MATH', '3C', 'Multivariable Calculus', 'calc_3'],
  ['MATH-16', 'MATH', '16', 'Introduction to Linear Algebra', 'linear_alg'],
  ['MATH-18', 'MATH', '18', 'Ordinary Differential Equations', 'diff_eq'],
  ['MATH-21', 'MATH', '21', 'Discrete Structures', 'discrete_math'],
  ['STAT-10', 'STAT', '10', 'Elementary Statistics', 'intro_stats'],
  ['PHYS-4A', 'PHYS', '4A', 'Physics for Scientists: Mechanics', 'phys_mech'],
  ['PHYS-4B', 'PHYS', '4B', 'Physics for Scientists: Electricity and Magnetism', 'phys_em'],
  ['PHYS-4C', 'PHYS', '4C', 'Physics for Scientists: Waves, Optics, Thermodynamics', 'phys_waves_thermo'],
  ['CHEM-1A', 'CHEM', '1A', 'General Chemistry I', 'gen_chem_1'],
  ['CHEM-1B', 'CHEM', '1B', 'General Chemistry II', 'gen_chem_2'],
  ['CHEM-12A', 'CHEM', '12A', 'Organic Chemistry I', 'organic_chem_1'],
  ['CS-111', 'CS', '111', 'Introduction to Programming Concepts', 'cs_1'],
  ['CS-112', 'CS', '112', 'Object-Oriented Programming', 'cs_2_oop'],
  ['CS-113', 'CS', '113', 'Data Structures', 'cs_3_data_structures'],
  ['CS-121', 'CS', '121', 'Computer Organization and Assembly', 'comp_arch_assembly'],
  ['ENGR-44', 'ENGR', '44', 'Introductory Circuit Analysis', 'engr_circuits'],
  ['ENGR-77', 'ENGR', '77', 'Computational Methods for Engineers', 'engr_programming'],
  ['BIOL-1A', 'BIOL', '1A', 'Cell and Molecular Biology', 'bio_cell_molec'],
  ['BIOL-1B', 'BIOL', '1B', 'Organismal Biology', 'bio_organismal'],
  ['ENGL-1A', 'ENGL', '1A', 'Composition and Reading', 'engl_comp_1'],
  ['ENGL-1B', 'ENGL', '1B', 'Critical Thinking and Composition', 'engl_comp_2'],
  ['COMM-1', 'COMM', '1', 'Public Speaking', 'public_speaking'],
].map(([key, prefix, number, title, concept]) => ({ key, prefix, number, title, concept }));

function projectCollegeEdges(courses) {
  const byConcept = new Map(courses.map(c => [c.concept, c]));
  const out = [];
  for (const c of courses) {
    const reqs = (CANONICAL_INDEX[c.concept] || {}).requires || [];
    for (const r of reqs) if (byConcept.has(r)) out.push({ from: byConcept.get(r).key, to: c.key });
  }
  return out;
}
const SAMPLE_COLLEGE_EDGES = projectCollegeEdges(SAMPLE_COLLEGE_COURSES);

/* ---- demo harness ---------------------------------------------------------- */
function PrereqGraphDemo() {
  const [mode, setMode] = useState('canonical');
  const tabs = [['canonical', 'Canonical model'], ['college', 'College mode (sample)']];
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
        <div style={{ display: 'flex', gap: 2, background: V.sunken, borderRadius: 999, padding: 3 }}>
          {tabs.map(([k, label]) => (
            <button key={k} onClick={() => setMode(k)} style={{
              border: 'none', cursor: 'pointer', fontFamily: 'inherit', fontSize: 13, fontWeight: mode === k ? 650 : 500,
              padding: '6.5px 15px', borderRadius: 999,
              background: mode === k ? V.primary : 'transparent',
              color: mode === k ? 'var(--color-on-primary, #F0FFE7)' : V.inkMuted,
            }}>{label}</button>
          ))}
        </div>
        <span style={{ fontSize: 12.5, color: V.inkSubtle }}>
          {mode === 'canonical' ? '41 concepts · edges from concept rules' : 'Sample college — 24 courses, edges projected from concept rules'}
        </span>
      </div>
      {mode === 'canonical'
        ? <PrereqGraph mode="canonical" concepts={CANONICAL_CONCEPTS} />
        : <PrereqGraph mode="college" courses={SAMPLE_COLLEGE_COURSES} edges={SAMPLE_COLLEGE_EDGES} conceptIndex={CANONICAL_INDEX} />}
    </div>
  );
}

if (typeof module !== 'undefined' && module.exports) {
  module.exports = { PrereqGraph, PrereqGraphDemo, computePrereqLayout, CANONICAL_CONCEPTS, SAMPLE_COLLEGE_COURSES, SAMPLE_COLLEGE_EDGES };
}
if (typeof window !== 'undefined') { window.PrereqGraph = PrereqGraph; window.PrereqGraphDemo = PrereqGraphDemo; }
