#!/usr/bin/env python3
"""Resumable statewide discovery and checkpoint tooling for Economics degrees.
...
``scripts/import_econ_as_degrees.py``. The historical ``import_as_degrees.py``
still belongs only to the CS extraction.

Typical use:

    python scripts/econ_as_research.py discover
    python scripts/econ_as_research.py status
    python scripts/econ_as_research.py next --limit 12
    python scripts/econ_as_research.py scaffold --college-id 2
...
RESEARCH_ROOT = HERE / "data" / "as_degrees_econ_research"
...
def fetch_json(url: str, timeout: int = 25):
    request = Request(
        ...
        headers={
            ...
            "User-Agent": "PMT-Economics-associate-research/1.0 (+local academic research)",
        },
    )
...
def category_url() -> str:
    return CATEGORY_ENDPOINT + "?" + urlencode({
        "degree_program_id": ECONOMICS_CATEGORY_ID,
        "search_string": "",
        "distance": 0,
        "orderby": "title",
    })
...
def initial_inventory(repo_rows: list[dict]) -> dict:
    ...
    return {
        "_meta": {
            ...
            "purpose": "Resumable statewide inventory for Economics associate-degree research.",
            "created_at": utc_stamp(),
            "last_checkpoint_at": None,
            "statewide_category_id": ECONOMICS_CATEGORY_ID,
...
def discover(args) -> int:
    ...
    if unmatched_statewide:
        ...
        raise ValueError(f"statewide Economics colleges do not map to a canonical repo college: {names}")
        ...
        for future in as_completed(future_to_row):
            ...
            try:
                ...
                filtered = [
                    ...
                    if str(program.get("degree_program_id")) == ECONOMICS_CATEGORY_ID
                ]
                row["programs"] = [
                    {
    ...
    return {
        "_meta": {
            ...
            "resume_note": (
                ...
                "one source for each "
                "general Economics AA-T, AS-T, AA, or AS; ignore business and other adjacent awards."
            ),
        },
        "inventory": {
...
def validate_research_record(path: Path, payload: dict, inventory_by_id: dict[int, dict]) -> list[str]:
    ...
    if meta.get("major_slug") != "econ":
        errors.append(f"{path}: _meta.major_slug must be econ")
...
def validate(args) -> int:
    ...
    if tmc_path.exists():
        ...
        if version_keys != {"economics_tmc"}:
            errors.append(
                "tmc_versions must contain only economics_tmc for the pinned cohort"
            )
...
def parser() -> argparse.ArgumentParser:
    ...
    discover_ap = sub.add_parser("discover", help="checkpoint the official statewide Economics inventory")
