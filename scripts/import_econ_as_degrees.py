#!/usr/bin/env python3
"""Compile completed 2025-2026 Economics degree research into the local app DB.
...
this one emits the generalized runtime identity instead:
    ...
    as_degree:<community_college_id>:econ:<ast|local_as|local_other>
...
Examples:
    ...
    python scripts/import_econ_as_degrees.py --college-id 2 --college-id 3
...
try:  # Supports both ``python scripts/...`` and import as a test module.
    ...
    import econ_as_research
    from import_as_degrees import build_course_index, merge_with_existing, transform_group


HERE = Path(__file__).resolve().parent
REPO_ROOT = HERE.parent
DEFAULT_COLLEGES_DIR = HERE / "data" / "as_degrees_econ_research" / "colleges"
DEFAULT_OUTPUT_JSON = HERE / "data" / "as_degrees_econ_compiled.json"
MAJOR_SLUG = "econ"
SLOTS = ("ast", "local_as", "local_other")
SOURCE_ACADEMIC_YEAR = econ_as_research.SOURCE_ACADEMIC_YEAR
        ...
        for degree in payload.get("degrees") or []:
            ...
            doc = {
                ...
                "analysis_blockers": sorted(set(
                    ["human verification required", "Economics degree template not yet configured"]
...
def write_compiled_artifact(path: Path, docs: list[dict], now: dt.datetime) -> None:
    ...
    payload = {
        "_meta": {
            ...
            "purpose": "Cumulative snapshot of Economics as_degree rows currently published to the local database.",
        },
        "documents": docs,
    }
...
def report(
    ...
    *,
    ...
    print(f"{verb} {len(docs)} Economics degree row(s) for {SOURCE_ACADEMIC_YEAR}.")
