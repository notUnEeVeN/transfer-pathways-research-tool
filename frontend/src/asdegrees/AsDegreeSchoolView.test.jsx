...
describe('AsDegreeSchoolView', () => {
  it('renders coverage, units, courses, and the catalog source', () => {
    ...
    expect(screen.getByText('Viewing degree')).toBeInTheDocument()
    expect(screen.getByText('College-specific Associate in Science (A.S.)')).toBeInTheDocument()
    expect(screen.getByRole('tab', { name: 'Local CS Degree' })).toHaveAttribute('aria-selected', 'true')
  ...
  it('shows a degree selector and switches when a college has several degrees', () => {
    ...
    expect(screen.getByRole('tab', { name: 'Local CS Degree' })).toBeInTheDocument()
    expect(screen.getByRole('tab', { name: 'CS Transfer Degree' })).toBeInTheDocument()
    expect(screen.getByText('Local CS A.S.', { selector: 'p' })).toBeInTheDocument()
    fireEvent.click(screen.getByRole('tab', { name: 'CS Transfer Degree' }))
  ...
  it('can isolate one degree type and suppress its repeated title', () => {
    ...
    expect(screen.queryByRole('tab', { name: 'Local CS Degree' })).not.toBeInTheDocument()
    expect(screen.getByRole('tab', { name: 'CS Transfer Degree' })).toHaveAttribute('aria-selected', 'true')
    expect(screen.getByText('Associate in Science for Transfer (A.S.-T)')).toBeInTheDocument()
  ...
  it('shows every allowed overlapping degree type and clearly identifies the active kind', () => {
    ...
    expect(screen.getByRole('tab', { name: 'CS Transfer Degree' })).toBeInTheDocument()
    expect(screen.getByRole('tab', { name: 'Local CS Degree' })).toBeInTheDocument()
    expect(screen.getByRole('tab', { name: 'Other Computing Degree' })).toBeInTheDocument()
    expect(screen.getByText('Associate in Science for Transfer (A.S.-T)')).toBeInTheDocument()
    fireEvent.click(screen.getByRole('tab', { name: 'Other Computing Degree' }))
    expect(screen.getByText('Another college-specific computing associate degree')).toBeInTheDocument()
