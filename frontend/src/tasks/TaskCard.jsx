...
/**
 * TaskCard — one task on the board. Title, assignee, a self-reported progress
 * bar, and a note count. Pure display + click; drag chrome lives in TaskBoard.
 */
export default function TaskCard({ task, onOpen }) {
  const notes = task.notes || []
  const progress = Math.max(0, Math.min(100, task.progress || 0))
  const isDone = task.status === 'done'
  const showBar = !isDone && progress > 0
      ...
      <div className='mt-2.5 flex items-center gap-2 min-h-[1.25rem]'>
        ...
        {notes.length > 0 && (
          <span className='ml-auto inline-flex items-center gap-1 text-tag text-ink-subtle'>
            <ChatBubbleLeftIcon className='w-3.5 h-3.5' />{notes.length}
          </span>
        )}
      </div>
    </div>
