...
describe('task history export', () => {
  ...
  it('wraps the same complete export in a factual AI cleanup prompt', () => {
    ...
    expect(briefing).toContain('Date | Actual hours | Work performed | Allocation note')
    expect(briefing).toContain('Date | Reported hours | Work description | Allocation note')
    expect(briefing).toContain('Weekly reconciliation totals')
    expect(briefing).toContain('Do not collect the usable notes only in a section at the bottom')
