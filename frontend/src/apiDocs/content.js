    ...
    endpoints: [
      {
        ...
        plain: 'Course, institution, equivalency, department, and import counts, including the stable public-university comparison denominator.',
        returns: '{ imported, courses, institutions, community_colleges, four_year, public_four_year, equivalencies, ... }',
      },
      {
        method: 'GET',
        path: '/va/institutions?level=four_year&cohort=schev_public_four_year',
        title: 'Virginia institution cohorts',
      ...
      {
        method: 'GET',
        path: '/va/courses?college=Northern%20Virginia%20Community%20College&limit=2000',
        title: 'Virginia courses',
        plain: 'Shared Virginia course rows with degree-building identifiers, optionally filtered by offering college, receiving university, department, or search text.',
        returns: '{ courses, total, skip, limit, receiver }',
        fields: [
          ['courses[].course_id', 'Stable project-minted numeric VCCS course id; use it in an A.S. degree option’s course_ids array. It is not an official VCCS identifier.'],
          ['courses[].course_key', 'Readable VCCS identity va:CODE; keep it positionally aligned with course_ids in course_keys.'],
          ['courses[].lands_as', 'With receiver=<university>, the first receiving target retained for backward compatibility. Its parent_id is usable in a four-year receiver; elective and non-credit buckets return null.'],
          ['courses[].landings[]', 'All stored targets at that receiver; some VCCS courses satisfy more than one university course. Refreshing the course import recovers secondary targets dropped by older imports.'],
          ['lands_as.code / landings[].code', 'Normalized source identifier. It can still name an elective bucket when parent_id is null. The numeric parent_id is derived only from code and can repeat, so retain receiving-school context when joining.'],
          ['college / receiver / limit', 'Use college=<name> for offered VCCS courses or receiver=<name> for university landings. limit is capped at 2000; compare total with skip + courses.length and paginate when needed.'],
        ],
      },
      {
        method: 'GET',
        path: '/va/degrees?institution=Virginia%20Commonwealth%20University',
        title: 'Virginia degree requirements and course IDs',
        plain: 'Full editable A.S. or four-year requirement documents plus IDs for codes named by the collected document or explicitly requested by the researcher.',
        returns: '{ institution, owner_id, degrees, courses, university_courses, university_courses_by_id }',
        fields: [
          ...
          ['university_courses[].parent_id', 'Project-minted receiving id for every concrete university code named by the collected degree document, including codes not yet referenced by its requirement tree.'],
          ['university_courses_by_id', 'The same receiving catalog keyed by parent_id for direct requirement-ledger lookup; each value also carries its numeric parent_id.'],
          ['codes=CSCI211,CSCI212', 'Optional, maximum 500. For a known institution, mechanically resolves syntactically valid codes even when no degree document exists. identity_source=requested_code and document_named=false mean the ID does not verify course existence.'],
          ['document scope', 'Without codes=, identity lists are limited to the collected degree document and are empty when none exists; they are not a complete institution course catalog.'],
          ['school_id + parent_id', 'Use this pair as the receiving identity. The numeric ID is derived only from code, so the same value can recur at different universities and as a VCCS course_id.'],
          ['college_id + course_id', 'Use this pair for an institution-local A.S. course. scoped_course_key is the equivalent readable identity; course_id and course_key alone are retained only as legacy fields.'],
        ],
      },
      {
        method: 'PUT',
        path: '/va/degrees/:id',
        title: 'Save or verify one Virginia degree',
        plain: 'Saves the full A.S. or four-year requirement object. The server recalculates acceptance, stamps the signed-in researcher, and refuses a verification claim for a partial document.',
