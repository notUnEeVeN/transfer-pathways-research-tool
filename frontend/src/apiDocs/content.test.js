...
describe('ENDPOINT_GROUPS content invariants', () => {
  ...
  it('documents Virginia ids needed to build both sides of a degree', () => {
    ...
    const institutions = virginia.endpoints.find((endpoint) => endpoint.path.startsWith('/va/institutions?'))
    const degrees = virginia.endpoints.find((endpoint) => endpoint.path.startsWith('/va/degrees?'))
    const saveDegree = virginia.endpoints.find((endpoint) => endpoint.path === '/va/degrees/:id')
    expect(courses.fields.flat().join(' ')).toContain('course_id')
    expect(courses.fields.flat().join(' ')).toContain('landings[]')
    expect(institutions.fields.flat().join(' ')).toContain('15 SCHEV public four-year institutions')
    expect(institutions.fields.flat().join(' ')).toContain('other_four_year')
    expect(institutions.fields.flat().join(' ')).toContain('not_collected')
    ...
    expect(degrees.fields.flat().join(' ')).toContain('course_namespace')
    expect(degrees.fields.flat().join(' ')).toContain('scoped_course_key')
    expect(degrees.fields.flat().join(' ')).toContain('college_id + course_id')
    ...
    expect(degrees.fields.flat().join(' ')).toContain('collection_status')
    expect(saveDegree.fields.flat().join(' ')).toContain('reopens verification')
    expect(saveDegree.fields.flat().join(' ')).toContain('allowlisted')
  })
})
