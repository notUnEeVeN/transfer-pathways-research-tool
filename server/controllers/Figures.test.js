...
beforeEach(async () => {
  await db.collection('published_figures').deleteMany({});
...
describe('POST /publish', () => {
  ...
  it('republish updates content and updated_at but keeps created_at', async () => {
    ...
    const first = await db.collection('published_figures').findOne({ _id: 'coverage-heatmap' });
    await new Promise((r) => setTimeout(r, 5));
    await run(publish, fakeReq({ uid: 'u1' }, { body: validBody({ title: 'v2' }) }));
    const doc = await db.collection('published_figures').findOne({ _id: 'coverage-heatmap' });
  ...
  it('protects an existing slug from another teammate', async () => {
    await run(publish, fakeReq({ uid: 'u1' }, { body: validBody() }));
    const res = await run(publish, fakeReq({ uid: 'u2' }, { body: validBody({ title: 'Nope' }) }));
    expect(res.statusCode).toBe(403);
    expect((await db.collection('published_figures').findOne({ _id: 'coverage-heatmap' })).title)
      .toBe('Coverage heatmap');
  });
...
describe('GET /gallery/:slug/:format (download)', () => {
  it('serves the decoded bytes with the right content type and filename', async () => {
    ...
    expect(res.headers['Content-Disposition']).toContain('coverage-heatmap.pdf');
...
describe('PATCH /gallery/:slug (edit metadata)', () => {
  ...
  it("lets an admin edit another user's figure", async () => {
    ...
    expect((await db.collection('published_figures').findOne({ _id: 'coverage-heatmap' })).title).toBe('Admin edit');
  });
  ...
  it('forbids a non-owner non-admin, leaving the figure unchanged', async () => {
    ...
    expect(res.statusCode).toBe(403);
    expect((await db.collection('published_figures').findOne({ _id: 'coverage-heatmap' })).title).toBe('Coverage heatmap');
  });
