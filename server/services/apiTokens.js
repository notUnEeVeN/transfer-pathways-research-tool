...
async function uidForToken(auditDb, raw) {
  ...
  const entry = {
    at: Date.now(),
    // Retired runner credentials must never become durable user tokens.
    uid: doc?.ephemeral ? null : doc?.uid ?? null,
  };
...
async function ensureTokenIndexes(auditDb) {
  await auditDb.collection(COLLECTION).deleteMany({ ephemeral: true });
  const indexes = await auditDb.collection(COLLECTION).indexes();
  if (indexes.some((index) => index.name === 'expires_at_1')) {
    await auditDb.collection(COLLECTION).dropIndex('expires_at_1');
  }
  await auditDb.collection(COLLECTION).createIndex({ uid: 1, created_at: 1 });
}

module.exports = {
  looksLikeApiToken, uidForToken, createToken,
