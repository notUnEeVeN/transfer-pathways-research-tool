...
describe('setReleasedIds', () => {
  it('upserts one config doc with who/when and reflects immediately (cache invalidated)', async () => {
    ...
    expect(doc.visual_published_ids).toEqual(['paper-district-heatmap']);
...
describe('disabled ids', () => {
  ...
  it('round-trips: set → get, with who/when stamped on the shared doc', async () => {
    ...
    const doc = await db.collection('settings').findOne({ _id: 'app' });
    expect(doc.visual_hidden_ids).toEqual(['complexity', 'time-to-degree']);
    expect(doc.updated_by).toBe('admin-1');
  });
  ...
  it('preserves unrelated fields on the shared app settings document', async () => {
    await db.collection('settings').insertOne({ _id: 'app', visible_pairs: [{ school_id: 1, major: 'CS' }] });
    await setReleasedIds(db, ['coverage-heatmap'], 'admin');
    const doc = await db.collection('settings').findOne({ _id: 'app' });
    expect(doc.visible_pairs).toEqual([{ school_id: 1, major: 'CS' }]);
  });
